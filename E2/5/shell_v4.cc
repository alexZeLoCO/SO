...
bool ComandoInterno(int mi_argc, char *mi_argv[]) {
	cout << "Ejecutando comandoInterno()" << endl;
   ...
   if (strcmp(mi_argv[0], "echo")==0) {
	cout << "Ejecutando echo interno" << endl;
	   for (int i = 1 ; i < mi_argc; i++) {
		   cout << mi_argv[i] << " ";
	   }
		cout << endl;
	   return true;
   } // Del comando interno echo

   if (strcmp(mi_argv[0], "prompt")==0) {
	if (mi_argc == 1) {
		perror("USO: prompt <nuevo_prompt>");
	} else {
		el_prompt = mi_argv[1];
	}
	return true;
   } // Del comando interno prompt
  ...
  while (!cin.eof()) {
    ...
    if (n_args != 0) {
      ...
      if (!ComandoInterno(n_args, comando_argv)) { 
	     cout << "Se va a ejecutar un comando externo" << endl; 
	...
	cout << "Se ha ejecutado un comando" << endl;
   } // De if n_args != 0
  } // Del while (!eof)
}
